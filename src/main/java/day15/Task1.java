...
public class Task1 {
    public static void main(String[] args) {
        File csv = new File("F:/JavaProject/JavaMarathon2021/src/main/resources/shoes.csv");
        File txt = new File("F:/JavaProject/JavaMarathon2021/src/main/resources/missing_shoes.txt");
//        List<String> list = new ArrayList<>();

        try {
            PrintWriter wr = new PrintWriter(txt);
            Scanner scanner = new Scanner(csv);
            while (scanner.hasNextLine()) {
                String line = scanner.nextLine();
                String[] warehouseList = line.split(";");
//                list.add(line);
                if (Integer.parseInt(warehouseList[2]) == 0) {
                    wr.println(line);
                }
            }
            wr.close();
